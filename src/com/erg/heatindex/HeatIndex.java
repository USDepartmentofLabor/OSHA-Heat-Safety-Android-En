...
public class HeatIndex extends Activity {
    			...
    			try {
    				...
    				if (!checkIfNumber(input1)){
	    				...
	    				if (Double.parseDouble(input1)<80){     	
	    				     new AlertDialog.Builder(editText1.getContext())
	    					.setTitle("Alert")
	    					.setMessage("A heat index value can not be calculated for temperatures less than 80 degrees Fahrenheit.")
	    					.setNeutralButton("OK", new DialogInterface.OnClickListener() {
	    					      public void onClick(DialogInterface dialog, int which) {    					        
	    					    }
	    					})
    				...
    				if (!checkIfNumber(input2) && myValid1==1 && myValid11==1 ){
	    				...
	    				if (Double.parseDouble(input2)>100 && myValid1==1 && myValid11==1 ){        				   
	    				     new AlertDialog.Builder(editText2.getContext())
	    					.setTitle("Alert")
	    					.setMessage("A heat index value can not be calculated for relative humidities greater than 100%.")
	    					.setNeutralButton("OK", new DialogInterface.OnClickListener() {
	    					      public void onClick(DialogInterface dialog, int which) {    					        
	    					    }
	    					})
				...
				if (getGPS==1) {
    			 if (notify1==0){
	    			...
	    			if (myValue ==0){
	    					 new AlertDialog.Builder(current_submit_final.getContext())
	 						.setTitle("Notification")
	 						.setMessage("This service function has been removed per agreement with the 3rd party.")
	 						.setNeutralButton("OK", new DialogInterface.OnClickListener() {
	 						      public void onClick(DialogInterface dialog, int which) {    
	 					...
	 					.show(); 
	    					...
	    					if (dataValid==0){
		    					 new AlertDialog.Builder(current_submit_final.getContext())
			 						.setTitle("Notification")
			 						.setMessage("A heat index value can not be calculated for temperatures less than 80 degree Fahrenheit.")
			 						.setNeutralButton("OK", new DialogInterface.OnClickListener() {
			 						      public void onClick(DialogInterface dialog, int which) {    
	    				...
	    				//TextView  mytest = ((TextView) findViewById(R.id.testplace));
	    				//mytest.setText(testIndex);
	    				
	    			}   //end of try
	    			catch (Exception e) {
	    		            /* Display any Error to the GUI. */
	    		        	//	((EditText) findViewById(R.id.edit_input_temp)).setText("Error-Current: " + e.getMessage());
	    		            //  Log.e(DEBUG_TAG, "WeatherQueryError", e);
	    		    } 
				} //end of check GPS
    		}
      ...
      //new code, check day max set
		...
		if (size>0){
	        ...
	        if (intHourValue<10){
	        	...
	        	if (dataValid == 0){
	        		  myValue=1;     //myValue=1 means we got the weather data, but it is not valid for HI calculation
					  myMaxTime="";
				}    
		}        	
		else{
  ...
  //new method for showing temp and humidity when current temp is too low or all day is too low.
    ...
    public void setUIforCurrentMAXDataNotValid() {  
    	EditText  editText1 = ((EditText) findViewById(R.id.edit_input_temp));
        EditText  editText2 = ((EditText) findViewById(R.id.edit_input_humid)); 
     	//reset the input fields
 		Double myResult1 = new Double(valueTemp);
 	    Double myResult2 = new Double(valueHumid);
 	    editText1.setText(myResult1.toString());
        ...
        //set time block gone
        LinearLayout dayMaxBlock = (LinearLayout) findViewById(R.id.daymaxblock);
        dayMaxBlock.setVisibility(View.GONE);
    }
